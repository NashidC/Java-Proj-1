...
public class rsa {
    ...
    public static boolean testPrime(int n) {
        ...
        //--------- YOUR CODE BELOW ---------
        if(n < 2)  // Any number below 2 cannot be prime e.g. 1
            result = false; 
        else if(n == 2) // we know that 2 is a prime
            result = true; 
        else if(n % 2 == 0)// even numbers cannot be prime
            result = false; 
        else { 
            int squared = (int)Math.sqrt(n); 
            for(int k=3; k<=squared && result; k+=2) {
                if (n % k == 0) 
                    result = false; 
            }
        }
    ...
    public static RSAKeys genKeys() {
       ...
       // e = d = p = q = 1;  // Delete this dummy line as you complete your code
        ...
        p = genPrime();
        q = genPrime();
        while(q == p) //make sure that the two are not equal
            q = genPrime(); 
        int what = (p - 1) * (q - 1); //will help give the key e
        e = genCoPrime(what);
        int x = 1;
        while((x * what + 1) % e != 0) //opposite of int what
            x++;
        d = (x * what + 1) / e;
    ...
    public static int crack(int e, int c) {
        ...
        if(c % 2 == 0) {
            ...
            int bounds = (int)Math.sqrt(c);
            for(int i=3; i<=bounds && b==1; i++) {
                if(c % i == 0)
                    b = i;
            }
        }
...
/*programming observations: there was no absolute need to utilize time efficiency.
 *   works fine with testing inputs tht are very small.  
 *   i dont think it would work so well with massive huge numbers
 *   Eclipse, the IDE, says that having a class name the same as the 
 	*   java file is not recommended which is strange considering that you have taught us
 	*   the opposite.
 *  
 */